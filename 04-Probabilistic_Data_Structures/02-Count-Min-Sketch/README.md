...
5. Define the `sketch_count` action. In this action you should do the following for each register:
   ...
   defined. As a max (or module) value for the hash use the constant `SKETCH_BUCKET_LENGTH`. You must cast `SKETCH_BUCKET_LENGTH` to 32 bits while using it as the max argument of the `hash` function, as the argument accepts fixed-width parameters. **Important:** use the `HashAlgorithm.crc32_custom`: since
...
the function `flow_to_bytestream(flow)`, the input parameter has to be a list or tuple of the form ("10.0.1.1", "10.0.1.2", 5000, 7000). Note that the protocol is not needed and will be automatically added.
